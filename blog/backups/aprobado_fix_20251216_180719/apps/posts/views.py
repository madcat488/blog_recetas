...
def mis_publicaciones(request):
    if not puede_crear_post(request.user):
        messages.error(request, 'No tienes permisos para ver esta sección.')
        return redirect('posts:listar_posts')
    ...
    publicados_count = Post.objects.filter(autor=request.user, estado='publicado').count()
    borradores_count = Post.objects.filter(autor=request.user, estado='borrador').count()
    ...
    return render(request, 'post/mis_publicaciones.html', context)