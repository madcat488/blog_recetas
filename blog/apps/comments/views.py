...
def puede_moderar_comentarios(user):
    ...
    return user.is_authenticated and (
        ...
        user.is_staff or 
        getattr(user, 'colaborador', False)
    )
...
def agregar_comentario(request, post_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Usar username como autor
            if hasattr(request.user, 'nombre') and request.user.nombre:
                comentario.autor = f"{request.user.nombre} {request.user.apellido}"
            else:
                comentario.autor = request.user.username
            
            # Determinar estado según permisos
            if puede_moderar_comentarios(request.user) and post.autor == request.user:
            ...
            # Si es AJAX, devolver JSON con información completa
            if is_ajax:
                return JsonResponse({
                    'success': True,
                    'message': mensaje,
                    ...
                    'fecha_publicacion': comentario.fecha_publicacion.strftime('%d/%m/%Y %H:%M'),
                    'auto_aprobado': estado == 'aprobado',
                    'html': render_to_string('comments/_comentario_item.html', {
        ...
        messages.error(request, 'Error al publicar el comentario. ' + ' '.join(error_messages))
    
    # Si no es POST o hay otro error
    if is_ajax:
        return JsonResponse({
            'success': False,
            'error': 'Método no permitido o error desconocido'
        }, status=400)
...
def eliminar_comentario(request, comentario_id):
    ...
    puede_eliminar = (
        ...
        (getattr(request.user, 'colaborador', False) and comentario.post.autor == request.user)
    )
...
def moderar_comentario(request, comentario_id):
    ...
    estado_original = comentario.estado
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Si se aprobó, limpiar motivo de rechazo
            if comentario_moderado.estado == 'aprobado':
                comentario_moderado.motivo_rechazo = ''
            
            comentario_moderado.save()
            
            # Mensaje según acción
            if estado_original != comentario_moderado.estado:
                estado_display = comentario_moderado.get_estado_display()
                messages.success(request, f'✅ Comentario {estado_display.lower()} correctamente.')
            else:
                messages.info(request, 'ℹ️ No se realizaron cambios en el estado del comentario.')
    ...
    context = {
        ...
        'form': form,
        'es_moderador': True,
        'estado_actual': comentario.get_estado_display(),
        'estado_color': {
            'pendiente': 'warning',
            'aprobado': 'success',
            'rechazado': 'danger'
        }.get(comentario.estado, 'secondary'),
    }
...
def aprobar_comentario_rapido(request, comentario_id):
    ...
    # CORRECCIÓN: Pasar el usuario como parámetro
    if not comentario.puede_ser_moderado_por(request.user):  # <-- Aquí estaba el error
        return JsonResponse({
            'success': False,
    ...
    return JsonResponse({
        ...
        'moderado_por': request.user.username if hasattr(request.user, 'username') else request.user.email
    })
...
def api_comentarios(request, post_id):
    ...
    elif not (user.is_staff or user.is_superuser or getattr(user, 'colaborador', False)):
    ...
    elif getattr(user, 'colaborador', False) and not (user.is_staff or user.is_superuser):
    ...
    comentarios = comentarios_qs.order_by('-fecha_publicacion')
    
    data = []
    for comentario in comentarios:
        # CORRECCIÓN: Pasar el usuario como parámetro
        puede_moderar = comentario.puede_ser_moderado_por(user) if user.is_authenticated else False
        ...
        data.append({
            ...
            'contenido': comentario.contenido,
            'fecha_publicacion': comentario.fecha_publicacion.strftime('%d/%m/%Y %H:%M'),
            'fecha_publicacion_iso': comentario.fecha_publicacion.isoformat(),
            ...
            'puede_eliminar': user.is_authenticated and (
                ...
                user.is_staff or 
                user.is_superuser or
                (getattr(user, 'colaborador', False) and post.autor == user)
            ),
    ...
    return JsonResponse({
        ...
        'usuario': {
            ...
            'es_colaborador': getattr(user, 'colaborador', False) if user.is_authenticated else False,
...
class EliminarOpinion(LoginRequiredMixin, DeleteView):
    ...
    def test_func(self):
        ...
        return (
            ...
            user.is_staff or
            user.is_superuser or
            (getattr(user, 'colaborador', False) and comentario.post.autor == user)
        )
